// json_xml.cpp : Este archivo contiene la función "main". La ejecución del programa comienza y termina ahí.


#include <iostream>
#include <sstream>
#include <nlohmann/json.hpp>
#include "json_xml.h"
#include "CSVJson.h"
#include "CSVXml.h"


void testJson() {
    nlohmann::json doc;

    doc["curso"] = "Microservicios en C++";
    doc["horas"] = 25;
    doc["tecnologias"] = { "xml", "json", "rest", "soap" };

    std::cout << "Json: " << doc.dump(4) << std::endl;
    std::cout << "curso: " << doc["curso"] << std::endl;
}

void strJson() {
    // Se define una cadena Raw: con formato json
    std::istringstream ss(R"({"nombre":"Ana","edad":28,"intereses":["programacion","musica","senderismo"]})");
    nlohmann::json doc;

    // Se convierte a json: 
    ss >> doc;
    std::cout << "nombre: " << doc["nombre"] << std::endl;
    std::cout << doc << std::endl;

}

void testCSVJson() {
    CSVJson::conversorCSVToJson("..\\..\\..\\practicas\\ficheros\\pedidos_final.csv", 
                                "..\\..\\..\\practicas\\ficheros\\out\\pedidos.json");
}

void testCSVXml() {
    CSVXml::conversorCSVToXml("..\\..\\..\\practicas\\ficheros\\pedidos_final.csv", 
                              "..\\..\\..\\practicas\\ficheros\\out\\pedidos.xml");
}


int main()
{
    //testJson();
    //strJson();
    testCSVJson();
    testCSVXml();

}

// Ejecutar programa: Ctrl + F5 o menú Depurar > Iniciar sin depurar
// Depurar programa: F5 o menú Depurar > Iniciar depuración

// Sugerencias para primeros pasos: 1. Use la ventana del Explorador de soluciones para agregar y administrar archivos
//   2. Use la ventana de Team Explorer para conectar con el control de código fuente
//   3. Use la ventana de salida para ver la salida de compilación y otros mensajes
//   4. Use la ventana Lista de errores para ver los errores
//   5. Vaya a Proyecto > Agregar nuevo elemento para crear nuevos archivos de código, o a Proyecto > Agregar elemento existente para agregar archivos de código existentes al proyecto
//   6. En el futuro, para volver a abrir este proyecto, vaya a Archivo > Abrir > Proyecto y seleccione el archivo .sln
