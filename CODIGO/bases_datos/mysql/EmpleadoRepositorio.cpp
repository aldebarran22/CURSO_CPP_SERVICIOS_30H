...
{
	soci::statement st = (sql_.prepare << "delete from empleados where id= :id", soci::use(id));
	st.execute(true);
...
{
	soci::statement st = (sql_.prepare << "update empleados set nombre=:nombre,cargo=:cargo where id=:id",
